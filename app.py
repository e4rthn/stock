...
def inject_global_vars():
    ...
    return dict(
        ...
        now=datetime.now, # Pass datetime.now function itself
...
def load_data(filename):
    ...
    if not os.path.exists(filename):
        ...
        try:
            ...
            flash(f"ไม่สามารถสร้างไฟล์ข้อมูลเริ่มต้น {filename}", "danger")
        except Exception as e:
            print(f"Unexpected error creating file {filename}: {e}")
            flash(f"เกิดข้อผิดพลาดไม่คาดคิดในการสร้างไฟล์ {filename}", "danger")
        return default_data
...
def load_price_history():
    ...
    if not isinstance(history, list):
        ...
        flash(f"ไฟล์ประวัติราคา ({PRICE_HISTORY_FILE}) ไม่ใช่รูปแบบรายการ", "warning")
        return []
...
def add_sku():
    ...
    if request.method == 'POST':
        ...
        errors = {}
        def safe_int(val): return int(val) if val is not None and val != '' and val.isdigit() else None
        ...
        if not errors:
            ...
            elif skus_data_check is None: errors['general'] = "เกิดข้อผิดพลาดในการโหลดข้อมูล SKU เดิม"
        ...
        skus_data = load_skus()
        if skus_data is None: flash("ผิดพลาด: ไม่สามารถโหลดข้อมูล SKU เพื่อบันทึกได้", "danger"); return render_template('add_sku.html', **render_context)
...
def move_to_display():
    ...
    batches = inventory.get('batches', [])
    back_stock_batches = [b for b in batches if b.get('location') == 'back_stock' and b.get('quantity', 0) > 0]; available_skus_in_back = {}; aggregated_back_stock = {}
    if back_stock_batches:
        back_stock_batches.sort(key=lambda b: (b.get('expiry_date', ''), b.get('arrival_date', ''))); unique_skus = sorted(list(set(b['sku'] for b in back_stock_batches)))
        for sku_code in unique_skus:
            if sku_code in skus_data: available_skus_in_back[sku_code] = skus_data[sku_code]; aggregated_back_stock[sku_code] = sum(b['quantity'] for b in back_stock_batches if b['sku'] == sku_code)
    ...
    if request.method == 'POST':
        ...
        if current_inventory is None: flash("ผิดพลาด: โหลดสต็อกไม่ได้", "danger"); return render_template('move_to_display.html', **render_context)
        ...
        for original_index, batch_data in source_batches_info:
            if quantity_left_to_move <= 0: break; available_in_this_batch = batch_data['quantity']; move_from_this_batch = min(available_in_this_batch, quantity_left_to_move)
            if move_from_this_batch > 0:
                new_quantity_original = available_in_this_batch - move_from_this_batch; indices_to_update[original_index] = new_quantity_original
                new_display_batch = { "batch_id": str(uuid.uuid4()), "sku": sku_to_move, "quantity": move_from_this_batch, "location": "display", "arrival_date": batch_data['arrival_date'], "expiry_date": batch_data['expiry_date'] }
                batches_to_add.append(new_display_batch); moved_count += move_from_this_batch; quantity_left_to_move -= move_from_this_batch
        if moved_count > 0:
            final_batches = [];
            ...
            final_batches.extend(batches_to_add); current_inventory['batches'] = final_batches; save_inventory(current_inventory); sku_name = skus_data.get(sku_to_move, {}).get('name', sku_to_move); flash(f"ย้าย '{sku_name}' จำนวน {moved_count} ชิ้น ไปยังหน้าร้านเรียบร้อยแล้ว", "success")
        elif requested_quantity > 0: flash("เกิดข้อผิดพลาด: ไม่สามารถย้ายสินค้าได้", "danger")
        else: flash("จำนวนต้องมากกว่า 0", "warning")
...
def record_sale():
    ...
    if skus_data is None or inventory is None: flash("ผิดพลาด: โหลดข้อมูลไม่ได้", "danger"); return redirect(url_for('index'))
    batches = inventory.get('batches', [])
    sellable_locations = ['rtc2', 'rtc1', 'display']; location_sell_priority = {loc: i for i, loc in enumerate(sellable_locations)}; sellable_batches = [b for b in batches if b.get('location') in sellable_locations and b.get('quantity', 0) > 0]; available_skus_to_sell = {}; aggregated_sellable_stock = {}
    if sellable_batches:
        ...
        for sku_code in unique_skus:
            if sku_code in skus_data: available_skus_to_sell[sku_code] = skus_data[sku_code]; aggregated_sellable_stock[sku_code] = sum(b['quantity'] for b in sellable_batches if b['sku'] == sku_code)
            else: print(f"!!! Warning: SKU '{sku_code}' sellable but not in skus.json !!!")
    render_context = {'skus': available_skus_to_sell, 'sellable_stock_agg': aggregated_sellable_stock, 'errors': {}, 'form_data': {}}
    if request.method == 'POST':
        form_data = request.form.to_dict(); render_context['form_data'] = form_data
        sku_to_sell = form_data.get('sku'); quantity_str = form_data.get('quantity'); errors = {}
        ...
        for original_index, batch_data in source_batches_info:
            if quantity_left_to_sell <= 0: break; available_in_this_batch = batch_data['quantity']; sell_from_this_batch = min(available_in_this_batch, quantity_left_to_sell)
            if sell_from_this_batch > 0: new_quantity_original = available_in_this_batch - sell_from_this_batch; indices_to_update[original_index] = new_quantity_original; sold_count += sell_from_this_batch; quantity_left_to_sell -= sell_from_this_batch
        if sold_count == requested_quantity and sold_count > 0:
            ...
            current_inventory['batches'] = final_batches; save_inventory(current_inventory); sku_name = skus_data.get(sku_to_sell, {}).get('name', sku_to_sell); flash(f"บันทึกการขาย '{sku_name}' จำนวน {sold_count} ชิ้น เรียบร้อยแล้ว", "success")
